...
$(document).ready(() => {
  loadTopTenTasks(sortArray(storageArray()));
...
const loadTopTenTasks = (storedObjArray) => {
  let storageNumber = 0;
  let displayArray = [];

  storedObjArray.forEach((e) => {
    if (!(e.completed) && storageNumber < 10) {
      displayArray.push(e);
      storageNumber++
    }
  })
...
const showMoreTasks = () => {
  ...
  completedArray.forEach((e) => {
    $(".task-container").prepend(e);
  })
}
...
$(".filter-buttons-container").on("click", (e) => {
  ...
  loadTopTenTasks(sortArray(filterArray));
})
